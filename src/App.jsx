  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!hasSupabaseConfig || !supabase) {
        setError("Login is nog niet gekoppeld. Probeer het later opnieuw.");
        setLoading(false);
        return;
      }
      ...
      if (mode === "signup") {
        const { error } = await supabase.auth.signUp({
          email,
          password,
          options: { emailRedirectTo: "https://www.lonelyheartsclub.nl/" }
        });
        if (error) throw error;
        const { error: waitlistError } = await supabase.from("waitlist").upsert({ email });
        if (waitlistError) console.warn("Waitlist opslaan overgeslagen:", waitlistError.message);
